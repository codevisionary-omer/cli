...
func TestLogsDirPrecedenceAndResolution(t *testing.T) {
	...
	absLogsDir := filepath.Join(tmpDir, "dapr-abs-logs")

	assert.NoError(t, os.Mkdir(webappDir, 0o755))
	assert.NoError(t, os.Mkdir(backendDir, 0o755))

	// Quote the absolute path so backslashes on Windows are valid YAML.
	runFilePath := filepath.Join(tmpDir, "dapr.yaml")
	runFile := fmt.Sprintf(`version: 1
common:
...
apps:
  ...
  - appDirPath: ./backend/
    ...
    logsDir: %q
`, absLogsDir)
	assert.NoError(t, os.WriteFile(runFilePath, []byte(runFile), 0o644))
	...
	expectedCommonLogsDir := filepath.Join(tmpDir, "central-logs")
	// app_2's relative logsDir is resolved against its appDirPath.
	expectedApp2LogsDir := filepath.Join(webappDir, "custom-logs")
	...
	t.Run("app's absolute logsDir is used as is", func(t *testing.T) {
		assert.Equal(t, absLogsDir, apps[2].LogsDir)
		assert.Equal(t, absLogsDir, apps[2].GetLogsDir())
	})
